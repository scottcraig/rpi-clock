...
export async function getStaticProps() {
  async function loadSelection() {
    ...
    await fetch("http://0.0.0.0:3000/flex-toggle", {
      ...
      .then(async (toggle) => {
        ...
        await fetch("http://0.0.0.0:3000/flex-index", {
          headers: {
            cache: "no-cache",
      ...
      .catch((e) => {
        console.error(e);
        console.log("toggle not detected");
        value = "0";
      });
  ...
  // if flex-index has no index in it
  if (selection.length < 1) {
    console.log("Missing index in flex-index file");
    selection = "0";
  }
...
export default function Home(props) {
  ...
  const { data } = useSWR(
    props.response.id === "000"
      ? "http://0.0.0.0:3000/api/backup"
  ...
  const response = sched.schedule[props.selection].values;

  useEffect(() => {
    for (const property in response) {
      if (
        sched.schedule[props.selection].values[property].start <
        new Date().toLocaleTimeString(["fr-FR"])
      ) {
        setSfx(sched.schedule[props.selection].values[property].sfx);
        setColour(sched.schedule[props.selection].values[property].colour);
        setBlock(sched.schedule[props.selection].values[property].text);
        setNext(
          sched.schedule[props.selection].values[parseInt(property) + 1]
            ?.text ?? null
        );
        setCountdown(sched.schedule[props.selection].values[property].end);
      }
    }
  });
    ...
    <>
      ...
      <div className="select-none absolute bottom-0 right-0 text-white font-medium text-xl m-7">
        <span className="text-shadow">
          {next?.toLowerCase()?.includes("lunch")
            ? "Lunch"
            : `Next ${
                next?.toLowerCase()?.includes("break") ? "break" : "block"
              }`}{" "}
          at: {countdown}
        </span>
      </div>
